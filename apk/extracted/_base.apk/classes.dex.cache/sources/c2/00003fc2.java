...
public class C9561x extends C9560w {
    ...
    public static <T> Set<T> m14001j(Set<? extends T> set, Iterable<? extends T> elements) {
        ...
        if (m14088A.isEmpty()) {
            m14071G0 = C9553r.m14071G0(set);
            return m14071G0;
        } else if (m14088A instanceof Set) {
    ...
    public static <T> Set<T> m14000k(Set<? extends T> set, T t) {
        int m6758d;
        C9612q.m13917h(set, "<this>");
        m6758d = C11888u.m6758d(set.size());
        LinkedHashSet linkedHashSet = new LinkedHashSet(m6758d);
        boolean z = false;
        for (T t2 : set) {
            boolean z2 = true;
    ...
    public static <T> Set<T> m13999l(Set<? extends T> set, Iterable<? extends T> elements) {
        ...
        m6758d = C11888u.m6758d(size);
        LinkedHashSet linkedHashSet = new LinkedHashSet(m6758d);
        linkedHashSet.addAll(set);
        C9550o.m14085y(linkedHashSet, elements);
        return linkedHashSet;
    }
    ...
    public static <T> Set<T> m13998m(Set<? extends T> set, T t) {
        int m6758d;
        C9612q.m13917h(set, "<this>");
        m6758d = C11888u.m6758d(set.size() + 1);
        LinkedHashSet linkedHashSet = new LinkedHashSet(m6758d);
        linkedHashSet.addAll(set);
        linkedHashSet.add(t);
        return linkedHashSet;
