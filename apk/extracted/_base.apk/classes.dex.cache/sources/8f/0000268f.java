...
public class JSTouchDispatcher {
    ...
    private void dispatchCancelEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher) {
        if (this.mTargetTag == -1) {
            C13677a.m1870H(ReactConstants.TAG, "Can't cancel already finished gesture. Is a child View trying to start a gesture from an UP/CANCEL event?");
            return;
        }
        C5743a.m24584b(!this.mChildIsHandlingNativeGesture, "Expected to not have already sent a cancel for this gesture");
        ...
        ((EventDispatcher) C5743a.m24583c(eventDispatcher)).dispatchEvent(TouchEvent.obtain(surfaceId, i, touchEventType, motionEvent, j, fArr[0], fArr[1], this.mTouchEventCoalescingKeyHelper));
    }
    ...
    public void handleTouchEvent(MotionEvent motionEvent, EventDispatcher eventDispatcher) {
        ...
        if (action == 0) {
            if (this.mTargetTag != -1) {
                C13677a.m1854j(ReactConstants.TAG, "Got DOWN touch before receiving UP or CANCEL from last gesture");
            }
            ...
            if (this.mTargetTag == -1) {
                C13677a.m1854j(ReactConstants.TAG, "Unexpected state: received touch event but didn't get starting ACTION_DOWN for this gesture before");
            } else if (action == 1) {
                ...
                if (this.mTouchEventCoalescingKeyHelper.hasCoalescingKey(motionEvent.getDownTime())) {
                    ...
                    C13677a.m1854j(ReactConstants.TAG, "Received an ACTION_CANCEL touch event for which we have no corresponding ACTION_DOWN");
                }
                this.mTargetTag = -1;
                this.mGestureStartTime = Long.MIN_VALUE;
            } else {
                C13677a.m1870H(ReactConstants.TAG, "Warning : touch event was ignored. Action=" + action + " Target=" + this.mTargetTag);
            }
        }
    }
